...
app/api/gallery/[id]/ai-picks/route.ts       → "עזרי לי לבחור" - Claude מנתח עד 60 תמונות ומסמן "אולי" את הטובות ביותר, כנקודת פתיחה ללקוחה
```
...
## "עזרי לי לבחור" - נקודת פתיחה עם AI

`app/api/gallery/[id]/ai-picks/route.ts` - כפתור בגלריית הלקוחה שמפעיל את
Claude (`claude-haiku-4-5-20251001`, אותו מודל כמו "עיצוב הגלריה עם AI"
בהגדרות הצלמת) כדי לסמן אוטומטית "אולי" על התמונות שנראות הכי טובות מבחינה
טכנית (חדות, עיניים פקוחות, הבעות פנים, קומפוזיציה) - שימושי בעיקר בגלריות
גדולות (מאות תמונות) שבהן קשה לדעת מאיפה להתחיל. לא דורס בחירות קיימות
(מדלג על תמונות שכבר מסומנות), ולא נוגע בבחירה הסופית - זו עדיין החלטה של
הלקוחה, ה-AI רק מציע נקודת פתיחה.

מגבלות טכניות מכוונות: מנתחים עד 60 תמונות (דגימה אחידה על פני כל הגלריה,
לא רק הראשונות) במקבצים של 15 בקריאות AI מקבילות, על גרסאות מוקטנות
(500px) כדי לשמור על זמן/עלות סבירים. תקרה יומית של 5 הרצות לכל צלמת
(`photographers.ai_picks_count/ai_picks_date`, אותו דפוס בדיוק כמו
`theme_gen_count/date` של עיצוב הגלריה) - רשת ביטחון על העלות, לא נעילה
אמיתית.
